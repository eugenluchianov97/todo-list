...
export default (props:ShowItemProps) => {
    ...
    const [date , setDate] = useState<any>(props.item.date);
    const [time , setTime] = useState<any>(props.item.time);
        ...
        <>
            <div className="w-4/5 sm:w-4/6 bg-white relative shadow-xl shadow-slate-300">
                ...
                <div className="text-xs p-2">
                    <button onClick={update} className="my-1 outline-none border bg-slate-700 text-white rounded-sm p-2 w-full">Сохранить</button>
                </div>
            </div>
        </>
    )
